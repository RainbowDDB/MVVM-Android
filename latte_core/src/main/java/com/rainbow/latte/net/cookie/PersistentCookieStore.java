...
public class PersistentCookieStore implements CookieStore {
    ...
    private void clearExpired() {
        ...
        for (String key : this.cookies.keySet()) {
            ...
            for (ConcurrentHashMap.Entry<String, Cookie> entry :
                    Objects.requireNonNull(cookies.get(key)).entrySet()) {
    ...
    public void add(HttpUrl httpUrl, Cookie cookie) {
        ...
        prefsWriter.putString(hostKey,
                TextUtils.join(",", Objects.requireNonNull(cookies.get(hostKey)).keySet()));
        // 保存cookie
    ...
    private boolean remove(String hostKey, Cookie cookie) {
        String name = this.cookieName(cookie);
        if (this.cookies.containsKey(hostKey) &&
                Objects.requireNonNull(this.cookies.get(hostKey)).containsKey(name)) {
            ...
            // 保存httpUrl对应的所有cookie的name
            prefsWriter.putString(hostKey, TextUtils.join(",",
                    Objects.requireNonNull(this.cookies.get(hostKey)).keySet()));
